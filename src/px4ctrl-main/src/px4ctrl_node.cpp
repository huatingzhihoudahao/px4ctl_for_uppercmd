...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber start_pose_sub =
        nh.subscribe<geometry_msgs::PoseStamped>("/start_pose",
                                         100,
                                         boost::bind(&Start_pose_Data_t::feed, &fsm.start_pose_data, _1),
                                         ros::VoidConstPtr(),
                                         ros::TransportHints().tcpNoDelay());
    ...
    //                                                   ros::TransportHints().tcpNoDelay());
    ros::Subscriber cmd_sub =
        nh.subscribe<std_msgs::Float64MultiArray>("cmd",
                                                      100,
                                                      boost::bind(&Command_Data_t::feed, &fsm.cmd_data, _1),
                                                      ros::VoidConstPtr(),
                                                      ros::TransportHints().tcpNoDelay());
    ...
    fsm.at_w_setpt_pub = nh.advertise<mavros_msgs::VehicleThrustAccSetpoint>("/at_w_setpt", 10);
    ...
    fsm.offboard_heartbeat_pub = nh.advertise<mavros_msgs::OffboardControlMode>("/offboard_heartbeat", 10); // debug
