...
Controller::Controller(Parameter_t &param_) : param(param_)
{
  ...
  hkust_Kpp.diagonal() = Eigen::Vector3d(param.gain_hkust.Kpp0, param.gain_hkust.Kpp1, param.gain_hkust.Kpp2);
	hkust_Kpi.diagonal() = Eigen::Vector3d(param.gain_hkust.Kpi0, param.gain_hkust.Kpi1, param.gain_hkust.Kpi2);
	hkust_Kpd.diagonal() = Eigen::Vector3d(param.gain_hkust.Kpd0, param.gain_hkust.Kpd1, param.gain_hkust.Kpd2);

	hkust_Kvp.diagonal() = Eigen::Vector3d(param.gain_hkust.Kvp0, param.gain_hkust.Kvp1, param.gain_hkust.Kvp2);
	hkust_Kvi.diagonal() = Eigen::Vector3d(param.gain_hkust.Kvi0, param.gain_hkust.Kvi1, param.gain_hkust.Kvi2);
	hkust_Kvd.diagonal() = Eigen::Vector3d(param.gain_hkust.Kvd0, param.gain_hkust.Kvd1, param.gain_hkust.Kvd2);

	hkust_Kv.diagonal() = Eigen::Vector3d(param.gain_hkust.Kv0, param.gain_hkust.Kv1, param.gain_hkust.Kv2);
	hkust_Ka.diagonal() = Eigen::Vector3d(param.gain_hkust.Ka0, param.gain_hkust.Ka1, param.gain_hkust.Ka2);
...
/*** 这个函数就是控制器，用于修改要下传送的 px4 commands  ***/
...
quadrotor_msgs::Px4ctrlDebug Controller::update_alg3(
    ...
    double voltage)
{
 // Check the given velocity is valid.  // Check the given velocity is valid.
  if (des.v(2) < -3.0)
    ROS_WARN("[px4ctrl] Desired z-Velocity = %6.3fm/s, < -3.0m/s, which is dangerous since the drone will be unstable!", des.v(2));

  // Compute desired control commands
  const Eigen::Vector3d pid_error_accelerations = hkust_computePIDErrorAcc(odom, des, param);
...
/****************** hkust computePID ErrorAcc ************/
Eigen::Vector3d Controller::hkust_computePIDErrorAcc(
    const Odom_Data_t &odom,
    const Desired_State_t &des,
    const Parameter_t &param)
{
  ...
  Eigen::Vector3d e_p, e_pd, e_v, e_vd, F_des;
  if(!is_odom_init){
		last_des_p = des.p;
		last_des_v = des.v;
		last_odom_p = odom.p;
  ...
  // ----------------position loop--------------
	const std::vector<double> p_integration_output_limits = {0.4, 0.4, 0.4};

	e_p = des.p - odom.p;
  // e_p(0)=std::isnan(des.p(0)) ? 0.0 : std::max(std::min(des.p(0) - odom.p(0), 1.0), -1.0);
	// e_p(1)=std::isnan(des.p(1)) ? 0.0 : std::max(std::min(des.p(1) - odom.p(1), 1.0), -1.0);
	// e_p(2)=std::isnan(des.p(2)) ? 0.0 : std::max(std::min(des.p(2) - odom.p(2), 1.0), -1.0);
	
  Eigen::Vector3d u_p_p = wRc * hkust_Kpp * cRw * e_p;
	e_pd = ((des.p - last_des_p) - (odom.p - last_odom_p)) * param.ctrl_rate;
	Eigen::Vector3d u_p_d = wRc * hkust_Kpd * cRw * e_pd;
	for (size_t k = 0; k < 3; ++k) {
		if (std::fabs(e_p(k)) < 0.5) {
			int_e_p(k) += e_p(k) * 1.0 / param.ctrl_rate;
		}
	}	
	Eigen::Vector3d u_p_i = wRc * hkust_Kpi * cRw * int_e_p;
	for (size_t k = 0; k < 3; ++k) {
		if (std::fabs(u_p_i(k)) > p_integration_output_limits[k]) {
			uav_utils::limit_range(u_p_i(k), p_integration_output_limits[k]);
			ROS_INFO("Position integration saturate for axis %zu, value=%.3f", k, u_p_i(k));
		}
	}
	Eigen::Vector3d u_p = u_p_p + u_p_i + u_p_d;
	// ----------------position loop end--------------
	pos_ctrl_output_queue.push_back(u_p);
	while (pos_ctrl_output_queue.size() > 20)
	{
		pos_ctrl_output_queue.pop_front();
	}
	u_p.setZero();
	for(auto pos_ctrl_output_queue_i : pos_ctrl_output_queue){
		u_p += pos_ctrl_output_queue_i;
	}

	u_p *= (1.0 / pos_ctrl_output_queue.size());
	
	// ----------------velocity loop--------------
	const std::vector<double> v_integration_output_limits = {0.4, 0.4, 0.4};

	Eigen::Vector3d des_v = des.v + u_p;

	e_v = des_v - odom.v;
	Eigen::Vector3d u_v_p = wRc * hkust_Kvp * cRw * e_v;

	e_vd = ((des_v - last_des_v) - (odom.v - last_odom_v)) * param.ctrl_rate;
	Eigen::Vector3d u_v_d = wRc * hkust_Kvd * cRw * e_vd;

	// const std::vector<double> integration_enable_limits = {0.1, 0.1, 0.1};
	for (size_t k = 0; k < 3; ++k) {
		if (std::fabs(e_v(k)) < 0.2) {
			int_e_v(k) += e_v(k) * 1.0 / param.ctrl_rate;
		}
	}

	
	Eigen::Vector3d u_v_i = wRc * hkust_Kvi * cRw * int_e_v;
	for (size_t k = 0; k < 3; ++k) {
		if (std::fabs(u_v_i(k)) > v_integration_output_limits[k]) {
			uav_utils::limit_range(u_v_i(k), v_integration_output_limits[k]);
  ...
  debug.des_v_x = (des.v(0) + Kp(0) * u_p(0)); //debug
  debug.des_v_y = (des.v(1) + Kp(1) * u_p(1));
  debug.des_v_z = (des.v(2) + Kp(2) * u_p(2));
    ...
    const Parameter_t &param)
{
  ...
  // cout << "des_p: " << des.p(0) << " " << des.p(1) << " " << des.p(2) << endl;
  // cout << "odom_p: " << odom.p(0) << " " << odom.p(1) << " " << odom.p(2) << endl;
  // cout << "des_v: " << des.v(0) << " " << des.v(1) << " " << des.v(2) << endl;
...
/* 被声明为一个常量成员函数，在调用该函数时，不会修改该对象的任何状态 */
