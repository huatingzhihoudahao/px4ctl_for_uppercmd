...
RC_Data_t::RC_Data_t()
{
    ...
    is_hover_mode = true;
    positon_manual_offboard_mode=-999;
...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    // fastlab
    mode = ((double)msg.channels[4] - 1000.0) / 1000.0;//hover
    gear = ((double)msg.channels[5] - 1000.0) / 1000.0;//offboard
    reboot_cmd = ((double)msg.channels[7] - 1000.0) / 1000.0;//arm
    // std::cout << "mode : " << mode <<"API_MODE_THRESHOLD_VALUE"<<API_MODE_THRESHOLD_VALUE<< std::endl;
    // std::cout << "gear : " << gear <<"GEAR_SHIFT_VALUE"<<GEAR_SHIFT_VALUE<< std::endl;
    // std::cout << "reboot_cmd : " << reboot_cmd <<"REBOOT_THRESHOLD_VALUE"<<REBOOT_THRESHOLD_VALUE<< std::endl;
...
void Odom_Data_t::feed(nav_msgs::OdometryConstPtr pMsg)
{
    ...
    rcv_stamp = now;
    recv_new_msg = true;
    //将信息pMsg里面的信息提取出来，分别存储到 p,v,q,w 中
...
#define VEL_IN_BODY 
    ...
    //不停地进行累加，只有当大于1s的时候才会进行比较
    if ( (now - last_clear_count_time).toSec() > 1.1 )//1
    {
        if ( one_min_count < 110 )//change original 100
        {
...
Start_pose_Data_t::Start_pose_Data_t()
{
    rcv_stamp = ros::Time(0);
        recv_new_msg = false;
        reached_start_pose=false;
        getmsg_want_toggle=false;
}

void Start_pose_Data_t::feed(geometry_msgs::PoseStampedConstPtr pMsg)
{
    msg = *pMsg;
    recv_new_msg = true;
    getmsg_want_toggle=true;
    reached_start_pose=false;
    std::cout<<"    !!!!!!!!!!!!!!!!recv_new_msg = true;getmsg_want_toggle=true;reached_start_pose=false!!!!!!;"<<std::endl;
...
void Command_Data_t::feed(std_msgs::Float64MultiArrayConstPtr pMsg)
{
    msg = *pMsg;
    rcv_stamp = ros::Time::now();
    std::array<double, 4> ref_at_w = {msg.data[0],msg.data[1], msg.data[2], msg.data[3]};
    ft=ref_at_w[0];
    wx=ref_at_w[1];
    wy=ref_at_w[2];
    wz=ref_at_w[3];
