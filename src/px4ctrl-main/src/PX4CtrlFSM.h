...
public:
	...
	Start_pose_Data_t start_pose_data;
	Imu_Data_t imu_data;
	...
	ros::Publisher ctrl_FCU_pub,at_w_setpt_pub,offboard_heartbeat_pub;
...
private:
	...
	Desired_State_t get_hover_des_with_planner_start_pose();
	// Desired_State_t get_cmd_des();

	Desired_at_w_t get_cmd_des();
	// ---- auto takeoff/land ----
	...
	void publish_at_w_from_mpc(const Desired_at_w_t &des_at_W, const ros::Time &stamp);
	
	void publish_acc_closeloop_ctrl(const Desired_at_w_t &des_at_W, const ros::Time &stamp);
