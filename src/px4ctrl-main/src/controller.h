...
struct Controller_Output_t
{
	...
	// 以飞行器自身的朝向为基准的角速度。 即飞行器在机体坐标系中测量的角速度。单位是弧度/s
...
public:
	...
	/* hkust */
	Eigen::Matrix3d hkust_Kpp;
	Eigen::Matrix3d hkust_Kpi;
	Eigen::Matrix3d hkust_Kpd;
	Eigen::Matrix3d hkust_Kv;
	Eigen::Matrix3d hkust_Kvp;
	Eigen::Matrix3d hkust_Kvi;
	Eigen::Matrix3d hkust_Kvd;
	Eigen::Matrix3d hkust_Ka;



	Eigen::Vector3d int_e_v;
	Eigen::Vector3d int_e_p;
	...
	/* hkust*/
	Eigen::Vector3d hkust_computePIDErrorAcc(
    	const Odom_Data_t &odom, const Desired_State_t &des,
    	const Parameter_t &param);
	...
	double get_thrust_0_1_from_ref_ft(const double ft) const;
...
private:
	...
	Eigen::Vector3d last_des_p, last_des_v, last_odom_p, last_odom_v;
	std::deque<Eigen::Vector3d> pos_ctrl_output_queue;
	bool is_configured;
