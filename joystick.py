...
from mavros_msgs.msg import RCIn as rc
...
def joy_callback(data):
    global pubRC, msg
    ( yaw, throttle,mode, roll,pitch,command, _,_) = data.axes
    (b0, b1, b2, b3, b4, b5, _, _, _, _,_) = data.buttons
    msg = rc()
    msg.channels.append ( convert_joy_units(roll * -1))
    msg.channels.append( convert_joy_units(pitch * -1))
    msg.channels.append ( convert_joy_units(throttle * -1))
    msg.channels.append ( convert_joy_units(yaw * -1))

    msg.channels.append ( convert_joy_units(mode * -1)) #4
    msg.channels.append ( convert_joy_units(command * -1))#5
    msg.channels.append (0)#6
    msg.channels.append (0)#7
#    if b0 > 0:
...
def init_joy_control():
    ...
    pubRC = rospy.Publisher('mavros/rc/in', rc, queue_size=10)
    rate = rospy.Rate(10)
    print("in init")
